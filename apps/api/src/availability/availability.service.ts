...
export class AvailabilityService {
  ...
  async getTutorTimezone(tutorId: string): Promise<{ timezone: string } | null> {
  ...
  private getReferenceDateInTz(dayOfWeek: number, timezone: string): Date {
    ...
    const todayStr = parts.find((p) => p.type === 'weekday')?.value?.toLowerCase() || '';
    ...
    const refUtc = Date.UTC(Number(year), Number(month) - 1, Number(day) + diff);
