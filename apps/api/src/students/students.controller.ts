...
export class StudentsController {
  ...
  async getLessons(@CurrentUser() user: AuthenticatedUser) {
    ...
    return lessons.map((l) => ({
      id: l.id,
      tutorName: l.tutor ? `${l.tutor.firstName} ${l.tutor.lastName}` : 'Unknown',
      subject: '',
  ...
  async getFavoriteTutors(@CurrentUser() user: AuthenticatedUser) {
    ...
    const tutorIds = [...new Set(lessons.map((l) => l.tutorId).filter(Boolean))];

    if (tutorIds.length === 0) return [];

    const tutors = await this.tutorProfileRepository.find({
      where: { userId: tutorIds as any, status: CourseStatus.APPROVED },
      relations: { user: true },
    });
