...
describe('Financial Audit (e2e)', () => {
  ...
  it('executes financial flow', async () => {
    ...
    const studentRes = await request(app.getHttpServer())
      ...
      .send({ email: studentEmail, password: 'Test1234', firstName: 'Student', lastName: 'Audit', role: UserRole.STUDENT });
    ...
    const tutorRes = await request(app.getHttpServer())
      .post('/api/v1/auth/register')
      .send({ email: tutorEmail, password: 'Test1234', firstName: 'Tutor', lastName: 'Audit', role: UserRole.TUTOR });
    let tutorToken = tutorRes.body.accessToken;
    ...
    await dataSource.query(
      ...
      [tutorId, 'Audit Tutor', 'Audit', '{English}', 25.00, CourseStatus.APPROVED, 0, 0]
    );

    // 2. Fund student balance to exactly $100.00
    await dataSource.query(`UPDATE student_profiles SET balance = 100.00 WHERE "userId" = $1`, [studentId]);
    const studentBalanceQ1 = await dataSource.query(`SELECT balance FROM student_profiles WHERE "userId" = $1`, [studentId]);
    ...
    const availRes = await request(app.getHttpServer())
      ...
      .send({ dayOfWeek: 1, startTime: '09:00', endTime: '15:00', isRecurring: true });
    ...
    const studentBalanceQ2 = await dataSource.query(`SELECT balance FROM student_profiles WHERE "userId" = $1`, [studentId]);
    console.log('Student Balance After Booking 1 (Raw PSQL):', studentBalanceQ2);
    ...
    const bookOverlapRes = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${studentToken}`)
      .send({ tutorId, scheduledTime, durationMinutes: 50 });
    console.log('Book Overlapping Response:', bookOverlapRes.status, JSON.stringify(bookOverlapRes.body));
    ...
    console.log('Book Adjacent Response:', bookAdjRes.status, JSON.stringify(bookAdjRes.body));
    ...
    const studentBalanceQ3 = await dataSource.query(`SELECT balance FROM student_profiles WHERE "userId" = $1`, [studentId]);
    ...
    await dataSource.query(`UPDATE lessons SET "scheduledTime" = $1, "endTime" = $2 WHERE id = $3`, [new Date(Date.now() - 3600000).toISOString(), new Date(Date.now() - 1000).toISOString(), lesson3Id]);
    ...
    const verifyQ1 = await dataSource.query(`SELECT "platformFee", price FROM lessons WHERE id = $1`, [lesson3Id]);
    console.log('Lesson 3 Verification (Raw PSQL):', verifyQ1);

    const tutorBalanceQ = await dataSource.query(`SELECT balance, "totalHoursTaught" FROM tutor_profiles WHERE "userId" = $1`, [tutorId]);
    console.log('Tutor Balance After Completion (Raw PSQL):', tutorBalanceQ);

    const studentBalanceQ4 = await dataSource.query(`SELECT balance FROM student_profiles WHERE "userId" = $1`, [studentId]);
    console.log('Student Balance After Completion (Raw PSQL):', studentBalanceQ4);
