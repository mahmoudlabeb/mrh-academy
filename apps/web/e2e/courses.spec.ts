...
test.describe("Courses Flow", () => {
  ...
  test("should navigate to course detail when clicking a course", async ({
    ...
    if (count > 0) {
      ...
      await expect(
        page.locator("text=Enroll Now").or(page.locator("text=سجل الآن")),
      ).toBeVisible({ timeout: 5000 });
    }
  });

  test("should redirect unauthenticated users to login for enrollment", async ({
    page,
  }) => {
    ...
    if (count > 0) {
      ...
      const enrollBtn = page
        .locator("text=Login to Enroll")
        .or(page.locator("text=سجل الدخول للتسجيل"));
