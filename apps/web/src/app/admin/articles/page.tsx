...
export default function AdminArticlesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(t("هل تريد حذف هذا المقال؟", "Delete this article?"))) return;
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label
                  ...
                  style={{ color: "var(--text-main)" }}
                >
                  {t("المحتوى (Markdown)", "Content (Markdown)")}
                </label>
                <button
                  type="button"
                ...
                >
                  {showPreview ? t("تعديل", "Edit") : t("معاينة", "Preview")}
                </button>
              </div>
              {showPreview ? (
                <div
                  className="prose prose-sm max-w-none rounded-xl p-5 min-h-[250px]"
                  style={{
                ...
                <textarea
                  placeholder={t(
                    "اكتب محتوى المقال باستخدام Markdown...",
                    "Write the article using Markdown...",
                  )}
                  value={content}
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              >
                ...
                />
                <div
                  className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${isPublished ? "translate-x-5" : ""}`}
                />
              </div>
              <span
                className="text-sm font-medium"
            ...
            <div className="flex gap-3 pt-2">
              ...
              >
                {createMutation.isPending || updateMutation.isPending ? (
                  ...
                  t("تحديث المقال", "Update article")
                ) : (
                  t("إنشاء المقال", "Create article")
                )}
              </button>
              {editing && (
                <button
        ...
        <div className="space-y-3">
          {articles.length === 0 && (
            <div className="card p-12 text-center">
              ...
              >
                {t("أنشئ مقالك الأول أعلاه", "Create your first article above")}
              </p>
            </div>
          )}
