    ...
    <div className="space-y-6">
      <div>
        ...
        >
          ...
          {pendingReviews.length > 0 && (
            <span
              className="me-2 px-2 py-0.5 rounded-full text-xs"
              style={{
        ...
        {pendingReviews.length === 0 ? (
          <div className="card p-8 text-center">
            <p style={{ color: "var(--text-muted)" }}>
              {lang === "ar" ? "لا توجد تقييمات معلقة" : "No pending reviews"}
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      >
                        {lang === "ar" ? "على" : "on"} {review.tutorName}
                      </span>
                    </div>
                    <div className="flex items-center gap-0.5 mb-2">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <svg
                          key={i}
                          className="w-4 h-4"
                          fill={i < review.rating ? "var(--signal)" : "none"}
                          viewBox="0 0 24 24"
                          stroke="var(--signal)"
      ...
      <div>
        ...
        {approvedReviews.length === 0 ? (
          <div className="card p-8 text-center">
            <p style={{ color: "var(--text-muted)" }}>
              {lang === "ar" ? "لا توجد تقييمات معتمدة" : "No approved reviews"}
            </p>
          </div>
        ) : (
