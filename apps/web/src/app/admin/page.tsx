...
const tabIcons: Record<string, React.ReactNode> = {
  ...
  tutors:    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" /></svg>,
  students:  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342" /></svg>,
  lessons:   <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" /></svg>,
  reports:   <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>,
  employees: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>,
...
export default function AdminPage() {
  ...
  const renderTab = () => {
    ...
    switch (activeTab) {
      ...
      case 'payouts': return <AdminPayoutsPage />;
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: isActive ? '1px solid rgba(212, 163, 83,0.2)' : '1px solid transparent',
                }}
              >
                <span className="shrink-0">{tabIcons[tab.id]}</span>
                <div className="min-w-0">
                  <div className="truncate">{tab.label}</div>
                  <div className="text-[10px] opacity-60 truncate">{tab.labelEn}</div>
                </div>
              </button>
            );
          })}
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <button
                  ...
                  style={{ background: profileOpen ? 'rgba(212, 163, 83,0.12)' : 'transparent' }}
                >
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm" style={{ background: '#D4A353' }}>
                    {user?.firstName?.[0] || 'A'}
                  </div>
                  <svg className={`w-4 h-4 transition-transform ${profileOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: '#E4CC9C' }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                </button>
                {profileOpen && (
                  <>
                    <div className="fixed inset-0 z-10" onClick={() => setProfileOpen(false)} />
                    <div className="absolute left-0 top-full mt-1 w-56 z-20 rounded-xl overflow-hidden animate-scale-in" style={{ background: '#1D535B', border: '1px solid #1D535B', boxShadow: '0 12px 24px rgba(0,0,0,0.4)' }}>
                      <div className="px-4 py-3 border-b border-[#1D535B]">
                        <p className="text-sm font-semibold" style={{ color: '#FFFFF0' }}>{user?.firstName} {user?.lastName}</p>
                        <p className="text-xs" style={{ color: '#E4CC9C' }}>{user?.email}</p>
                      </div>
                      <div className="p-1">
                        <button
                          onClick={logout}
