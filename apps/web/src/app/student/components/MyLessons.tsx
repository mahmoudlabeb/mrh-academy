        ...
        {subTabs.map((st) => (
          <button
            ...
            className={`flex-1 px-4 py-2.5 text-sm font-medium rounded-lg transition-all ${
              subTab === st.key ? "shadow-sm" : "hover:bg-white/5"
            }`}
            style={{
              background: subTab === st.key ? "var(--bg-main)" : "transparent",
              color:
                subTab === st.key ? "var(--text-main)" : "var(--text-muted)",
              border:
                subTab === st.key
        ...
        <div>
          {loadingLessons ? (
            ...
            <div className="space-y-3">
              {lessons.map((lesson) => {
                const cfg =
                  statusConfig[lesson.status] ?? statusConfig.confirmed;
                return (
                  <div
                    key={lesson.id}
                    className="card p-5 flex items-center justify-between gap-4 hover:translate-y-0"
                  >
          ...
          {loadingLessons ? (
            ...
            <div className="card p-12 text-center">
              ...
              <p
                ...
                style={{ color: "var(--text-muted)" }}
              >
                {t("لا توجد دروس مجدولة", "No scheduled lessons")}
              </p>
            </div>
          ) : (
            <div className="card p-6">
              <div className="grid grid-cols-7 gap-1 mb-4">
                ...
                {Array.from({ length: new Date(calendarDays[0]).getDay() }).map(
                  (_, i) => (
