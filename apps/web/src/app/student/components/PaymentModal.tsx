...
export default function PaymentModal({
  ...
  const paymentMethods = PAYMENT_METHODS.filter((method) =>
    enabledMethodKeys.has(method.key),
  );
  const method =
    paymentMethods.find((item) => item.key === activeMethod) ??
    paymentMethods[0];
  const requiresReceipt = method?.requiresReceipt ?? false;

  const amountNum = parseFloat(amount) || 0;
  const amountInUsd = currency === "EGP" ? amountNum / egpRate : amountNum;
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      if (!method)
        throw new Error(
          t("لا توجد وسيلة دفع متاحة", "No payment method is available"),
        );
    ...
    onError: (
      ...
      const msg =
        ...
        error?.message ||
        t("حدث خطأ أثناء إرسال الدفع", "Payment error occurred");
      alert(msg);
    },
  });
      ...
      >
        ...
        <div className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!paymentMethodsQuery.isLoading &&
              ...
              paymentMethods.length === 0 && (
                ...
                >
                  {t(
                    "لا توجد وسيلة دفع متاحة حالياً",
          ...
          {requiresReceipt && (
            ...
            >
              {t(
                "سيتم مراجعة طلب الدفع من قبل الإدارة. ستتلقى إشعاراً عند الموافقة.",
                "Your payment will be reviewed by admin. You will be notified upon approval.",
              )}
            </p>
          )}
        </div>
