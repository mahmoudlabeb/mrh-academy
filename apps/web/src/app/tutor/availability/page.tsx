...
export default function AvailabilityPage() {
  ...
  const saveTimeOff = () => {
    ...
    localStorage.setItem(
      "tutor_time_off",
      JSON.stringify({ start: timeOffStart, end: timeOffEnd }),
    );
  ...
  const addMutation = useMutation({
    mutationFn: async (slotData: {
      ...
      const { data } = await apiClient.post<Slot>(
        "/tutor/availability",
        slotData,
      );
      return data;
    ...
    onSuccess: (newSlot) => {
      queryClient.setQueryData<Slot[]>(["tutor-availability"], (prev) =>
        prev ? [...prev, newSlot] : [newSlot],
      );
    },
    onError: (error) => {
  ...
  const slotAt = useCallback(
    (dayIndex: number, hour: number): Slot | undefined => {
      const time = `${String(hour).padStart(2, "0")}:00`;
      return slots.find(
        (s) =>
          s.dayOfWeek === dayIndex &&
          s.startTime.slice(0, 2) === time.slice(0, 2),
      );
    },
    [slots],
  );
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[var(--canvas)] flex items-center justify-center">
        <svg
          className="w-8 h-8 text-[var(--signal)] animate-spin"
          viewBox="0 0 24 24"
          fill="none"
        >
          <circle
    ...
    >
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              >
                {isAr ? "أوقات التوفر" : "My Availability"}
              </h1>
              <p className="mt-1" style={{ color: "var(--text-muted)" }}>
                {isAr
                  ? "اسحب على الجدول لتحديد ساعات التدريس المتاحة"
                  : "Click and drag across the grid to set your teaching hours"}
              </p>
            </div>
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  className={`w-10 h-5 rounded-full transition-colors relative ${isRecurring ? "bg-[var(--signal)]" : "bg-[var(--border-strong)]"}`}
                >
                  <input
                    type="checkbox"
                    checked={isRecurring}
                  ...
                  />
                  <div
                    className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${isRecurring ? "translate-x-5" : ""}`}
                  />
                </div>
              ...
              <Link href="/tutor" className="btn-secondary px-4 py-2 text-sm">
                Dashboard
              </Link>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {searchParams.get("mode") === "timeoff" && (
          <section
            className="card mb-6 p-5 border"
          ...
          >
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                <h2
                  ...
                  style={{ color: "var(--text-main)" }}
                >
                  {isAr ? "خطة الإجازة" : "Plan time off"}
                </h2>
                <p
                ...
                >
                  {isAr
                    ? "احفظ فترة لن تظهر فيها مواعيد جديدة للطلاب."
                    : "Save a break so you can keep your teaching calendar clear."}
                </p>
              </div>
            ...
            {timeOffSaved && (
              <p className="text-xs mt-3" style={{ color: "var(--success)" }}>
                {isAr ? "تم حفظ فترة الإجازة." : "Time off saved."}
              </p>
            )}
          </section>
        )}
            ...
            >
              ...
              {HOURS.map((hour, hourIndex) => (
                <div key={hour} className="contents">
                  <div
                    className="text-xs pe-2 text-end py-1.5 border-b self-start sticky start-0 font-mono"
                    style={{
                      background: "var(--bg-light)",
                      borderColor: "var(--border-color)",
        ...
        <div className="flex items-center gap-6 mt-6 justify-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[var(--ink-muted)]">Available</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-[var(--canvas-sunken)] border border-[var(--border)]" />
            <span className="text-sm text-[var(--ink-muted)]">Unavailable</span>
          </div>
        </div>
      </div>
    </div>
