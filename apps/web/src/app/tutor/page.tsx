...
function TutorPageContent() {
  ...
  const connectQuery = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<{ connected: boolean; onboardingComplete: boolean; stripeAccountId: string | null }>('/stripe/connect/status');
      return data;
    },
    retry: false,
  });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="card-dark p-5">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'rgba(212, 163, 83,0.12)' }}>
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: 'var(--primary-color)' }} strokeWidth={1.5}>
        ...
        <header className="dashboard-header">
          <div className="flex items-center justify-between px-6 py-3">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Theme Toggle */}
              <button onClick={toggleTheme} className="p-2 rounded-xl transition-colors" style={{ color: '#FFFFF0' }} onMouseEnter={(e) => e.currentTarget.style.background = '#1D535B'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                {theme === 'dark' ? (
              ...
              <div className="relative">
                <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 p-1.5 rounded-xl transition-colors" style={{ color: '#FFFFF0' }} onMouseEnter={(e) => e.currentTarget.style.background = '#1D535B'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: 'rgba(212, 163, 83,0.2)', color: '#D4A353' }}>
                    {user?.firstName?.[0]}{user?.lastName?.[0]}
                  </div>
                  <svg className={`w-4 h-4 transition-transform ${profileOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
